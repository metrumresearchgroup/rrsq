...
Secrets or other information set as environment variables can be passed via the `.envs` arg for the `submit_job` method.
...
On your machine setting environment variables to secret values, or if they are set in the Renviron can be captured
by name and passed to the execution context
...
In the above example, the environment variables `ENV_SECRET` and `ANOTHER_SECRET` will also be set to the same values in the queue.
